...
- API 支持方法：`GET`、`POST`、`OPTIONS`
- 写入能力：可选，默认因 `Read-Only Mode` 处于开启状态而关闭；只有在 `Settings → Local API` 中关闭 `Read-Only Mode` 后才可用
...
| --- | --- | --- |
| `200` | 成功 | 请求正常完成 |
| `201` | 已创建 | `POST /api/v1/papers` 成功创建文献 |
| `204` | 无内容 | `OPTIONS` 预检 |
| `400` | 参数错误 | 请求格式错误、JSON 非法、标题为空、UUID 非法或 `paperType` 非法 |
| `403` | 禁止访问 | 非本机请求、来源不被允许 |
| `404` | 未找到 | 端点不存在，或文献 / 插件资源不存在 |
| `405` | 方法不允许 | 使用了 `PUT`、`PATCH`、`DELETE` 等不支持的方法 |
| `409` | 冲突 | 创建文献时检测到重复项 |
...
| 字段 | `GET /papers/{uuid}` 响应 | `POST /papers` 请求 | `POST /papers` 响应 | 说明 |
| --- | --- | --- | --- | --- |
| `id` | `id` | — | `id` | 稳定文献 UUID；也可据此拼出 `lattice://paper/{id}` |
| `citekey` | `citekey` | — | — | 只读引用键 |
| `title` | `title` | `title` | `title` | 读写流程都会出现 |
| `authors` | `authors[]` | `authors` | `authors` | 详情接口是 `string[]`；写入请求/响应是分号分隔字符串 |
| `year` | `year` | `year` | `year` | 共享核心字段 |
| `journal` | `journal` | `journal` | — | 创建时可传，详情接口会返回，但创建响应不会回显 |
| `abstract` | — | `abstract` | — | 创建时可写入，但当前读接口不会返回 |
| `doi` | `doi` | `doi` | `doi` | 共享核心标识符 |
| `volume` | `volume` | `volume` | — | 创建时可传，详情接口会返回 |
| `issue` | `issue` | `issue` | — | 创建时可传，详情接口会返回 |
| `pages` | `pages` | `pages` | — | 创建时可传，详情接口会返回 |
| `isbn` | `isbn` | `isbn` | — | 创建时可传，详情接口会返回 |
| `paperType` | `paperType` | `paperType` | — | 创建时可传，详情和搜索结果会返回 |
| `pdfPath` | — | `pdfPath` | — | 只写输入字段，用于尝试附加 PDF |
| `collections` | — | `collections[]` | — | 只写输入字段，按大小写不敏感方式匹配文献集路径 |
| `tags` | — | `tags[]` | — | 只写输入字段，标签名数组 |
| `enrich` | — | `enrich` | — | 只写输入字段，后台补全开关 |
| `pdfAttached` | — | — | `pdfAttached` | 只在创建响应里返回，表示 PDF 附加是否成功 |
| `enrichmentStatus` | — | — | `enrichmentStatus` | 只在创建响应里返回，值为 `pending` 或 `none` |
| `warnings` | — | — | `warnings[]` | 只在创建响应里返回，表示非致命 warning |
...
| `pdfAttached` | `boolean` | PDF 书签是否附加成功 |
...
| `409` | `{"error":"Duplicate paper found","existingPaperID":"uuid"}` | 检测到 DOI 或规范化标题重复 |
...
- 服务器允许的 CORS 方法是 `GET, POST, OPTIONS`
...
HTTP 请求总大小上限为 `262144` 字节（`256 KiB`），包含 headers 和 body。
