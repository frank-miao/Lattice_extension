...
| 文献创建 | 当 `/status.capabilities` 中包含 `create-paper` 时，可创建文献、分配文献集/标签、附带 PDF 并触发后台补全 |
...
- 所有业务端点都位于 `/api/v1` 之下。
- 写入能力是可选的，只有在 `Settings → Local API` 中关闭 `Read-Only Mode` 后，`POST /api/v1/papers` 才可用。如果请求里带了 `pdfPath`，该 PDF 还必须位于 `Trusted Folders` 或其子目录内。
